...
fn main() {
    let (inputs, outputs) = load_dataset_multi("Datas/Datas/table_4_14.csv", 2, 3, false)
        .expect("Failed to load dataset");

    let mut mlp = MLP::<Sigmoid>::new(&[10, 3], 2, -1.0..=1.0);

    println!("Avant entraînement : {:#?}", mlp);
    mlp.train(&inputs, &outputs, 1.2, 0.001, Some(2000));
    ...
    for (i, (input, output)) in inputs.iter().zip(outputs.iter()).enumerate() {
        ...
        if result == output.iter().position(|&x| x == 1.0).unwrap_or(usize::MAX) {
            correct += 1;
            println!(
                "{i}: {:.2?} => Output: {:?} (Expected: {:?})",
                input, result, output
